      ...
      return (
        <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3">
          ...
          <p className="text-sm text-gray-600">
            Sponsorships: <span className="font-semibold text-green-600">{payload[0].value}</span>
          </p>
        </div>
      );
    }
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
                ...
                {availableYears.map((year) => (
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-100 first:rounded-t-lg last:rounded-b-lg ${
                      year === selectedYear ? 'bg-blue-50 text-green-600' : 'text-gray-700'
                    }`}
                  >
                    {year}
                  </button>
          ...
          <div className="text-sm text-gray-500 mb-3 pl-4">
            Total Sponsorships  
            <span className={`ml-1 ${Number(percentageChange) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
