...
export default function PrivateChatInterface({
  ...
  const createPrivateRoom = async (
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:8080/api/private-chat/create-room',
        {
          method: 'POST',
          headers: {
  ...
  const loadPrivateRoomHistory = async (roomId: string, username: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/private-chat/room/${roomId}/history?username=${encodeURIComponent(
          username
        )}`
      );
      ...
      if (response.ok) {
        ...
        try {
          await fetch(
            `http://localhost:8080/api/private-chat/room/${roomId}/mark-read?username=${encodeURIComponent(
              username
            )}`,
            {
              method: 'POST',
      ...
      >
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => {
            const isOwnMessage =
              message.sender === username || message.senderName === username;
