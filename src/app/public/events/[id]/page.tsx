...
import { fetchSponsorshipsByEvent } from '@/services/publicUserService';
        ...
        if (eventData.sponsorshipEnabled) {
          try {
            const sponsorshipData: SponsorshipType[] =
              await fetchSponsorshipsByEvent(eventId);
  ...
  return (
    <div className="flex flex-col justify-start items-center flex-shrink-0">
      ...
      {loading ? (
        ...
        <>
          <Event event={event} sponsorshipNames={sponsorshipNames} />
        </>
      ) : null}
    </div>
  );
