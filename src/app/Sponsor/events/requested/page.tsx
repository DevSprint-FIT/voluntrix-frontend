...
export default function SponsorEventRequestsPage() {
  ...
  const [volunteers, setVolunteers] = useState<Map<number, Volunteer>>(
    new Map()
  );
      ...
      try {
        ...
        const volunteerPromises = Array.from(hostIds).map(async (hostId) => {
          try {
            ...
            console.error(
              `Error fetching volunteer for host ${hostId}:`,
              error
            );
          }
        });
      ...
      const fetchVolunteerInfo = async () => {
        try {
          ...
          if (cachedVolunteer) {
            ...
            const volunteerData = await sponsorService.getVolunteer(
          ...
          console.error(
            `Error fetching volunteer info for event ${eventId}:`,
            error
          );
          setVolunteer(null);
        } finally {
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500 py-8">
          ...
          <p className="text-gray-500">
            You haven't made any sponsorship requests yet.
          </p>
        </div>
      ) : (
