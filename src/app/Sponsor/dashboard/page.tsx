...
export default function SponsorDashboardPage() {
  ...
  const [allSponsorEventData, setAllSponsorEventData] = useState<
    SponsorEventData[]
  >([]);
  const [eventsLoading, setEventsLoading] = useState(false);

  const [totalEventsSponsored, setTotalEventsSponsored] = useState<number>(5);
  const [totalSponsorships, setTotalSponsorships] = useState<number>(5);
  const [totalEventsDonated, setTotalEventsDonated] = useState<number>(2);
  const [totalDonations, setTotalDonations] = useState<string>("LKR 250,000");
  ...
  const fetchSponsorEventData = async () => {
    setEventsLoading(true);
    try {
      // Get dummy sponsored event dates for calendar highlighting
      const dates = sponsorService.getSponsoredEventDates();
      setEventDates(dates);
      console.log(
        `Sponsor has ${dates.length} event dates to highlight on calendar`
      );

      // Get dashboard stats with dummy values
      const stats = sponsorService.getDashboardStats();
      setTotalEventsSponsored(stats.totalEventsSponsored);
      setTotalSponsorships(stats.totalSponsorships);
      setTotalEventsDonated(stats.totalEventsDonated);
      setTotalDonations(stats.totalSponsorshipAmount);
    } catch (error) {
  ...
  const fetchSponsorshipsData = async (year: number) => {
  ...
  useEffect(() => {
    async function fetchSponsorData() {
      try {
        // Fetch real sponsor profile data from backend
    ...
    fetchSponsorEventData();
    fetchSponsorData();
    fetchDonationsData(selectedYear);
    fetchSponsorshipsData(selectedYear);
  }, [selectedYear]);
    ...
    <div className="p-5 m-5">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div>
          ...
          <h1 className="text-2xl font-primary font-bold mt-1">
            Main Dashboard
          </h1>
        </div>
      ...
      <div className="flex flex-wrap gap-10 mb-8">
        ...
        />
        <StatCard
          title="Total Events Donated"
          value={totalEventsDonated}
          icon={<BarChart size={20} />}
        />
        <StatCard
      ...
      <div className="flex gap-6 mb-8">
        ...
        <div className="bg-[#FBFBFB] rounded-xl p-6 w-[25rem] h-[25rem]">
          <h2 className="text-lg font-secondary font-semibold mb-4">
            Sponsored Event Dates
          </h2>

          {/* Add CustomNavigation component here */}
          <CustomNavigation date={value} onNavigate={setValue} />
